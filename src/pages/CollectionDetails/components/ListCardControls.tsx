...
const ListCardControls: React.FC<
  ICollectionNft & {
    isApproved: boolean;
    refetchApprove: () => Promise<void>;
  }
> = ({ tokenId, isApproved, refetchApprove, contract_type, nftContract }) => {
  ...
  const handleApprove = async () => {
    try {
      ...
      if (contract_type === IContractType.ERC721) {
        ...
        await ApproveNFT(account, provider, chainId, nftContract);
      } else {
        const { ApproveNFTerc1155 } = await import("utils/methods");
        await ApproveNFTerc1155(account, provider, chainId, nftContract);
      }
      refetchApprove();
...
export default ListCardControls;
